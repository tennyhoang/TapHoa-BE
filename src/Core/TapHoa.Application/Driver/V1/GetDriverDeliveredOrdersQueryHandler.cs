...
public class GetDriverDeliveredOrdersQueryHandler(IRepository<Order> orderRepo)
    : IRequestHandler<GetDriverDeliveredOrdersQuery, Result<PagedResult<OrderResponse>>>
{
    public async Task<Result<PagedResult<OrderResponse>>> Handle(
        GetDriverDeliveredOrdersQuery request, CancellationToken cancellationToken)
    {
        ...
        var query = orderRepo.Query()
            ...
            .Where(o =>
                o.Status == OrderStatus.InHub_ReadyForPickup ||
                o.Status == OrderStatus.Completed)
            .Where(o => o.ShippingToHubAt.HasValue && o.ShippingToHubAt.Value.Date == today)
